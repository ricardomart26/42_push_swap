...
int simulate_next(stacks_t *temp, int *chunks, int moves)
{
    ...
    if (cmd[0].moves >= cmd[1].moves)
    {
        ...
        return (cmd[0].moves + moves);
    }
    else
    {
        printf("\n\t(simulate next) end is the best %d", cmd[0].moves);
        return (cmd[1].moves + moves);
    }
}
...
int check_four_opt(stacks_t main, int *chunks)
{
    ...
    cmd[0].moves += simulate_next(&temp, chunks, cmd[0].moves);
    ...
    cmd[1].moves = simulate_next(&temp, chunks, cmd[1].moves);
